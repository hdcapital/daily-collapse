...
def analyse(
    ...
    try:
        ...
        prompt = (
            ...
            "Search the web for whatever best explains the fall — any news from "
            "the day: financial results or guidance, ASX announcements, trading "
            "halts, capital raisings, broker downgrades, contract losses, drill "
            "results, sector-wide or macro moves, or anything else. If the "
            f"trigger was something {name} itself released on {date} (results, "
            "guidance, a trading update), also fill `highlights` and `lowlights` "
            "with the key positives and negatives from that release, figures "
            "included; otherwise leave both empty. Then answer in the required "
            "JSON."
        )
